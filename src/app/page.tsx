...
import { Swiper, SwiperSlide } from "swiper/react";// slider library
...
export default function HomePage() {
    ...
    const fetchFavorites = async () => {
        try {
            const res: any = await CallApi(
                "get",
                `${BaseUrl}api/products?page=1&favourite=1`,
                {},
                headers
            );
    ...
    const handelfavorit = async (id: number) => {
        try {
            ...
            const res: ApiResponse<Favorit> = await CallApi(
                "post",
                url,
                dataToSend,
                headers
                                ...
                                <div className="max-w-screen-xl mx-auto">
                                    ...
                                    <div className="w-full bg-white mt-6 relative shadow-sm rounded-xl">
                                        ...
                                        <div className="flex justify-center items-center text-2xl mb-4">
                                            <h2 className="text-btn-color font-bold text-[26px] sm:text-[30px] mb-4 tracking-tight">
                                                الأكثر مبيعاً
                                            </h2>
                                        </div>
                                            ...
                                            {data.topSelling?.map(
                                                (image, index) => (
                                                    <SwiperSlide key={index}>
                                                        <div className="px-1 transition-all duration-300 hover:scale-[1.02] hover:shadow-md">
                                                            <Card
                                                                {...image}
                                                                love={favoriteProducts.includes(
                                                                    image.id
                                                                )}
                                                                handellove={() =>
                                                                    handelfavorit(
                                    ...
                                    <div className="w-full bg-white mt-6 relative shadow-sm rounded-xl">
                                        ...
                                        <div className="flex justify-center items-center text-xl mb-4">
                                            <h2 className="text-btn-color font-bold text-[20px] sm:text-[30px] mb-4 tracking-tight">
                                                {" "}
                                                عروض مميزة
                                            </h2>
                                        </div>
                                    ...
                                    <div className="w-full bg-white mt-3">
                                        {data.categoriesWithProducts?.map(
                                            (item) => (
                                                <div
                                                    key={item.id}
